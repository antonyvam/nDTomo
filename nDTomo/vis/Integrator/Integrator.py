...
from nDTomo.vis.Integrator.DCalibration import Calibration, CreatMask

from nDTomo.vis.Integrator.PDFmask import CreatPDFMask

from nDTomo.vis.Integrator.CreateAzimJson import CreatAzimint

from nDTomo.vis.Integrator.ZigzagTomoInt_LiveRead import XRDCT_LiveRead
from nDTomo.vis.Integrator.ZigzagTomoInt_LiveMulti import XRDCT_ID15ASqueeze

from nDTomo.vis.Integrator.ContRotTomoInt_LiveRead import Fast_XRDCT_LiveRead
from nDTomo.vis.Integrator.ContRotTomoInt_LiveMulti import Fast_XRDCT_ID15ASqueeze

username = getpass.getuser()
print('User is %s' %username)

if username != 'opid15':
    from nDTomo.vis.Integrator.ScatterTomoInt import XRDCT_Squeeze
    from nDTomo.vis.Integrator.ContRotTomoInt_Live import Fast_XRDCT_LiveSqueeze
    from nDTomo.vis.Integrator.ZigzagTomoInt_Live import XRDCT_LiveSqueeze
else:
...
#                self.na = len(omega)
            ...
            try:
                with open(self.specfile) as f:
                    ...
                    for line in f:
                        args=line.split()
                        omega[n]=(float(args[aerox_UpPos])+float(args[aerox_DownPos]))/2.0 # 360.0
                        y[n]=(float(args[aeroy_UpPos])+float(args[aeroy_DownPos]))/2.0
                        dio[n]=float(args[mon])
                        etime[n]=float(args[TrigTime])                          
                        n=n+1
